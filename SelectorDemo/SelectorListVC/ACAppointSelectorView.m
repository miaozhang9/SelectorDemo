...
//        self.noteView.noteLabel.text =  @"注：由于客户未能在预约时间参与面核，请重新预约";//（今日时段不可预约）
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [UIColor pc_colorWithHexString:@"#4a4a4a"];
...
- (UILabel *)descLabel {
    if (!_descLabel) {
        ...
        _descLabel.textColor = [UIColor pc_colorWithHexString:@"#999999"];
...
- (void)initAndModifySubviews {
    ...
    [_noteLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(@14);
        make.right.and.top.and.bottom.equalTo(weakSelf);
...
- (UILabel *)noteLabel {
    if (!_noteLabel) {
        ...
        _noteLabel.textColor = [UIColor pc_colorWithHexString:@"#F53420"];
